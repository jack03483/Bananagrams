...
function generateSentence(word, def) {
  ...
  // Detect word form from ending and definition
  const endsED = w.endsWith('ed');
  const endsING = w.endsWith('ing');
  const endsLY = w.endsWith('ly') && w.length > 3;
  const endsS = w.endsWith('s') && !w.endsWith('ss') && w.length > 3;
  const endsNESS = w.endsWith('ness');
  const endsMENT = w.endsWith('ment');
  const endsTION = w.endsWith('tion') || w.endsWith('sion');
  const endsER = w.endsWith('er') && w.length > 3;
  const endsEST = w.endsWith('est') && w.length > 4;
  const isVerbInf = d.startsWith('to ');
  const isNounDef = d.startsWith('a ') || d.startsWith('an ') || d.startsWith('the ');

  let templates;

  if (endsED) {
    templates = [
      `She ${blank} it without thinking twice.`,
      `They ${blank} everything before leaving.`,
      `He ${blank} the whole thing in an afternoon.`,
      `Once she ${blank} it, there was no going back.`,
      `The artist ${blank} for hours until it was perfect.`,
    ];
  } else if (endsING) {
    templates = [
      `He spent the morning ${blank} in the garden.`,
      `She was ${blank} when the phone rang.`,
      `${blank} requires patience and practice.`,
      `They couldn't stop ${blank} once they started.`,
      `The sound of ${blank} filled the room.`,
    ];
  } else if (endsLY) {
    templates = [
      `She moved ${blank} through the crowded hall.`,
      `He spoke ${blank}, choosing each word with care.`,
      `The cat watched ${blank} from the windowsill.`,
      `The wind blew ${blank} across the open field.`,
      `She smiled ${blank} and turned away.`,
    ];
  } else if (endsNESS || endsMENT || endsTION) {
    templates = [
      `The ${blank} of the situation surprised everyone.`,
      `Her ${blank} was obvious to all who knew her.`,
      `They discussed the ${blank} at length.`,
      `The ${blank} lasted longer than anyone expected.`,
      `Without ${blank}, none of it would have mattered.`,
    ];
  } else if (endsER && !isNounDef) {
    templates = [
      `The ${blank} worked late into the night.`,
      `Every ${blank} in the group had a story.`,
      `She was the best ${blank} anyone had seen.`,
      `No ${blank} could have done it faster.`,
      `He called himself a ${blank} of sorts.`,
    ];
  } else if (endsEST) {
    templates = [
      `It was the ${blank} thing she had ever seen.`,
      `Of all of them, this one was the ${blank}.`,
      `He picked the ${blank} option without hesitating.`,
      `That was the ${blank} part of the whole ordeal.`,
    ];
  } else if (endsS && isNounDef) {
    templates = [
      `The ${blank} were arranged neatly on the shelf.`,
      `She collected ${blank} from every place she visited.`,
      `All the ${blank} had been accounted for.`,
      `He noticed the ${blank} as soon as he walked in.`,
      `A pile of ${blank} sat in the corner of the room.`,
    ];
  } else if (isVerbInf) {
    templates = [
      `She tried to ${blank} it as best she could.`,
      `They needed to ${blank} before the deadline.`,
      `Learning to ${blank} takes years of practice.`,
      `He promised to ${blank} everything by morning.`,
      `The plan was to ${blank} and hope for the best.`,
    ];
  } else if (isNounDef) {
    templates = [
      `The ${blank} sat on the table, untouched.`,
      `She picked up the ${blank} and turned it over.`,
      `He had never seen a ${blank} quite like it.`,
      `A single ${blank} was all that remained.`,
      `The old ${blank} had been in the family for generations.`,
    ];
  } else {
    // Likely adjective or general
    templates = [
      `The surface looked strangely ${blank}.`,
      `Everything about it felt ${blank}.`,
      `She described the scene as ${blank}.`,
      `It was more ${blank} than anyone expected.`,
      `The ${blank} quality of the light made her pause.`,
    ];
  }
